...
parser.add_argument('--save_dir', type=str, default='/Users/kevinmwongso/Documents/SMU MQF/qf603_Quantitative_Analysis_of_Financial_Market/PROJECT/InvariantStock-main/best_models', help='directory to save model')
parser.add_argument('--wandb', action='store_true', help='whether to use wandb')
parser.add_argument('--normalize', action='store_true', help='whether to normalize the data')
parser.add_argument('--device', default="mps",type=str, help='devices')
...
train_index = np.load("/Users/kevinmwongso/Documents/SMU MQF/qf603_Quantitative_Analysis_of_Financial_Market/PROJECT/InvariantStock-main/data/train_index.npy")
valid_index = np.load("/Users/kevinmwongso/Documents/SMU MQF/qf603_Quantitative_Analysis_of_Financial_Market/PROJECT/InvariantStock-main/data/valid_index.npy")
test_index = np.load("/Users/kevinmwongso/Documents/SMU MQF/qf603_Quantitative_Analysis_of_Financial_Market/PROJECT/InvariantStock-main/data/test_index.npy")
...
def main(args):
    ...
    for epoch in tqdm(range(args.num_epochs)):
        ...
        if predictor_root is not None:
            ...
            # print("Output columns after generate_prediction_scores:", output.columns)
            # # After generating output
            # print("Output shape:", output.shape)
            # print("Test Index shape:", test_index[:, -1])
            # print("Index to assign shape:", dataset.index[test_index[:, -1]].shape)

            # Ensure the length matches
            expected_length = dataset.index[test_index[:, -1]].shape[0]

            if len(output) > expected_length:
                output = output[:expected_length]  # Truncate if too many predictions
            elif len(output) < expected_length:
                # Pad with NaNs if too few predictions
                # output = pd.Series(np.pad(output, (0, expected_length - len(output)), constant_values=np.nan), index=dataset.index[test_index[:, -1]])
                output = np.pad(output, (0, expected_length - len(output)), constant_values=np.nan)

            output = pd.DataFrame(output, columns=['pred'], index=dataset.index[test_index[:, -1]])  # Assigning index after length check
            # Assign index
            output.index = dataset.index[test_index[:, -1]]

            
            # # Check if output is a DataFrame or Series; assign to `pred` column
            # if isinstance(output, pd.DataFrame) or isinstance(output, pd.Series):
            #     output['pred'] = output.values  # Assign predictions to 'pred' column if needed
            # else:
            # # If `generate_prediction_scores` returns a tensor or array, convert it
            #     output['pred'] = pd.Series(output, index=dataset.index[test_index[:, -1]])
            ...
            # # Ensure the length matches
            # if output.shape[0] == dataset.index[test_index[:, -1]].shape[0]:
            #     output.index = dataset.index[test_index[:, -1]]
            # else:
            #     output.index = dataset.index[test_index[:, -1][:2287]]

            # # Handle the mismatch accordingly, e.g., slice, pad, or adjust the indices
            ...
            print("No model was saved as best during training.")
    ...
    feature_mask.load_state_dict(torch.load(feat_mask_root))
    output = generate_prediction_scores(feature_mask,predictor,test_dataloader,args)
    print('FINAL PREDICTION STEP')

    output = np.array(output).flatten()
    expected_length = dataset.index[test_index[:, -1]].shape[0]

    if len(output) > expected_length:
        output = output[:expected_length]  # Truncate if there are too many predictions
    elif len(output) < expected_length:
        output = np.pad(output, (0, expected_length - len(output)), constant_values=np.nan)  # Pad if too few predictions

    # Convert output to DataFrame with appropriate length
    output = pd.DataFrame(output, columns=['pred'], index=dataset.index[test_index[:, -1]])

    # Assign the 'label' column for ranking calculation
    output["label"] = dataset.loc[output.index, 'label']
    print("Test Result:")
    rankic(output)
    # output.index = dataset.index[test_index[:,-1]]
...
if __name__ == "__main__":
    ...
    # Argument parsing, dataset loading, etc.
    dataset = multi_add_env(dataset)
