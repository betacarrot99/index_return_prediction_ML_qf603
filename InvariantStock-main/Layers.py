...
class Predictor(nn.Module):
    ...
    def forward(self, x, returns):
        ...
        # Compute ranks based on alpha_mu
        alpha_mu, _ = self.factor_decoder.alpha_layer(stock_latent)  # Use alpha_mu from the alpha layer

        # Ensure ranks is a 2D tensor with shape (batch_size, 1)
        ranks = torch.zeros((batch_size, 1), device=self.device)
        ranks_index = torch.argsort(alpha_mu, descending=True)  # Get descending order of alpha_mu

        # Populate ranks tensor with indices
        ranks[ranks_index, 0] = torch.arange(0, batch_size, device=self.device, dtype=torch.float32).view(-1, 1)
        ...
        all_ones = torch.ones(batch_size, 1).to(self.device)
        pre_pw_dif = torch.matmul(reconstruction, torch.transpose(all_ones, 0, 1)) - torch.matmul(all_ones, torch.transpose(reconstruction, 0, 1))
        # gt_pw_dif = torch.matmul(all_ones, torch.transpose(returns, 0, 1)) - torch.matmul(returns, torch.transpose(all_ones, 0, 1))
        # Ensure `returns` is a 2D tensor with shape [batch_size, 1]
        if returns.dim() == 1:
            returns = returns.unsqueeze(1)

        gt_pw_dif = (
        torch.matmul(all_ones, returns.T) -
        torch.matmul(returns, all_ones.T)
        )


        rank_loss = torch.mean(ranks * F.relu(pre_pw_dif * gt_pw_dif))
        vae_loss = reconstruction_loss + kl_divergence + rank_loss
        # return vae_loss, reconstruction_loss, rank_loss, kl_divergence, reconstruction, factor_mu, factor_sigma, pred_mu, pred_sigma
        return vae_loss, reconstruction_loss, rank_loss, kl_divergence, reconstruction, factor_mu, factor_sigma, pred_mu, pred_sigma, alpha_mu
